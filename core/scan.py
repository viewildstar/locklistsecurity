...
def run_scan_once(db: Session, access_token: str) -> ScanRun:
    ...
    # We call async Graph from sync FastAPI endpoints via asyncio.run
    ...
    async def _do() -> ScanRun:
        ...
        if not tenant:
            tenant = Tenant(aad_tenant_id=aad_tid or "unknown", display_name=display_name or "")
            db.add(tenant)
            db.commit()
            db.refresh(tenant)
        else:
        ...
        sr = ScanRun(tenant_id=tenant.id, status="running", started_at=datetime.utcnow())
        db.add(sr)
        db.commit()
        db.refresh(sr)
